...
yolov8_msgs__msg__Point2D__are_equal(const yolov8_msgs__msg__Point2D * lhs, const yolov8_msgs__msg__Point2D * rhs)
{
  ...
  // alreadyinside
  if (lhs->alreadyinside != rhs->alreadyinside) {
    return false;
  }
  return true;
}
  ...
  yolov8_msgs__msg__Point2D * output)
{
  ...
  // alreadyinside
  output->alreadyinside = input->alreadyinside;
  return true;
}
  ...
  yolov8_msgs__msg__Point2D__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    yolov8_msgs__msg__Point2D * data =
      (yolov8_msgs__msg__Point2D *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!yolov8_msgs__msg__Point2D__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          yolov8_msgs__msg__Point2D__fini(&output->data[i]);
        }
        return false;
      }
    }
